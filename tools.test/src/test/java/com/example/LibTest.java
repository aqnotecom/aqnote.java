...
public final class LibTest {
	...
	public static void main(String argv[]) {
		...
		if(argv == null || argv.length < 1) {
		    argv = new String[1];
		    argv[0] = "vpn.alibaba.com";
		}
		
		if (argv.length != 1)
			die("usage: LibTest <server_name>");

		System.out.println("OpenConnect version: " + LibOpenConnect.getVersion());
		System.out.println("  PKCS=" + LibOpenConnect.hasPKCS11Support() +
				   ", TSS=" + LibOpenConnect.hasTSSBlobSupport() +
				   ", STOKEN=" + LibOpenConnect.hasStokenSupport() +
				   ", OATH=" + LibOpenConnect.hasOATHSupport());
		lib.setReportedOS("win");
		lib.setLogLevel(LibOpenConnect.PRG_DEBUG);
