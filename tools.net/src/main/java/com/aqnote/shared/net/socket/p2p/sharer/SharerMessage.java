...
public class SharerMessage {
    ...
    /**
     * JOIN pid host port: Requests a peer to add the supplied host/port combination, associated with the node
     * identified by pid, to its list of known peers.
     */
    ...
    /**
     * QUER return-pid key ttl: Queries a peer to see if the peer has any record of a file name matching key. If so,
     * send a RESP message back to the node identified by return-pid; if not, propagate the query to all known peers
     * with a decreased ttl (time-to-live) value, unless ttl is already 0.
     */
    ...
    /**
     * REPL ...: Used to indicate an acknowledgment of the other message types above or to send back results of a
     * successful request.
     */
